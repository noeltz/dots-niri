...
while read -r img; do
    ...
    (
        if [[ "$img" =~ \.(gif|GIF)$ ]]; then
            magick "$img[0]" -strip -thumbnail 300x480^ -gravity center -extent 300x480 -quality 80 +repage "$cache_file" 2>/dev/null
        else
            magick "$img" -strip -thumbnail 300x480^ -gravity center -extent 300x480 -quality 80 +repage "$cache_file" 2>/dev/null
        fi
